    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 ApplyFlameDistortion(float3 positionOS, float t, float2 uv)
            {
                ...
                float n = noise21(uv * 4 + float2(0, t * _FlickerSpeed));
                float xOffset = (n - 0.5) * vertexDistance * _DistortionSize;
                float yOffset = sin(t * _FlickerSpeed + n*6) * vertexDistance * 0.25;
